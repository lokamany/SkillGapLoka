...
const Sidebar = ({ activeTab, setActiveTab }) => {
  ...
  const menuItems = [
    ...
    { id: 'communication', icon: Mic2, label: 'AI Communication' },
  ...
  return (
    <aside className="w-72 border-r border-slate-800 flex flex-col bg-[#020617] h-screen fixed left-0 top-0 z-50">
      ...
      <div className="p-6 border-t border-slate-800">
        <button onClick={logout} className="w-full flex items-center gap-4 px-4 py-3 text-red-400/70 hover:text-red-400 rounded-xl font-bold transition-all">
          <LogOut size={20} />
          <span>Sign Out</span>
        </button>
      </div>
